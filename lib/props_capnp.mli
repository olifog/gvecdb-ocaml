...
module CapnpWireFormat : sig
  ...
  val from_bytes :
    bigstring -> (Capnp.Message.ro Bigstring_message.Message.t, error) result
end

val register_node_schema :
  t ->
  ?txn:[> `Read | `Write ] Lmdb.Txn.t ->
  string ->
  int64 ->
  (unit, error) result

val register_edge_schema :
  t ->
  ?txn:[> `Read | `Write ] Lmdb.Txn.t ->
  string ->
  int64 ->
...
val get_node_meta :
  t -> ?txn:[> `Read ] Lmdb.Txn.t -> node_id -> (intern_id, error) result
