...
let field_type_is_pointer = function Text | Data -> true | _ -> false
      ...
      | Result.Ok msg -> (
          ...
          | Some node -> (
              ...
              | Schema.Reader.Node.Struct s ->
                  ...
                  let fields_raw =
                    Schema.Reader.Node.Struct.fields_get_list s
                  in
                  let fields =
                    List.filter_map
                      (fun field ->
                        ...
                        | Schema.Reader.Field.Slot slot -> (
                            ...
                            let offset_raw =
                              Stdint.Uint32.to_int
                                (Schema.Reader.Field.Slot.offset_get slot)
                            in
                            ...
                            | Some ft ->
                                ...
                                let dv =
                                  extract_default_value ft
                                    (Schema.Reader.Field.Slot.default_value_get
                                       slot)
                                in
                                Some
                                  {
...
let deserialize_field_descriptor (s : string) (pos : int ref) :
  ...
  let* default_value =
    ...
    | 4 ->
        ...
        pos := !pos + 4;
        Ok (Float32_default bits)
    | 5 ->
        let* () = need 8 in
        let bits = String.get_int64_le s !pos in
        pos := !pos + 8;
        Ok (Float64_default bits)
    | 6 ->
        let* () = need 2 in
...
let serialize_schema (dwc : int) (pc : int) (fields : field_descriptor list) :
  ...
  List.iter
    (fun fd -> Buffer.add_string buf (serialize_field_descriptor fd))
    fields;
  Buffer.contents buf

let deserialize_schema (s : string) :
    (int * int * field_descriptor list, error) result =
  if String.length s < 6 then Error (Corrupted_data "truncated schema header")
  else
...
let register_schema_from_capnp (db : t) ~(kind : Props_capnp.schema_kind)
    ~(type_name : string) ~(capnp_path : string) ~(struct_name : string) ?txn ()
  ...
  let* () =
    wrap_lmdb_exn (fun () ->
        let bs =
          Bigstring.of_string ~off:0 ~len:(String.length serialized) serialized
        in
        Lmdb.Map.set db.metadata ?txn schema_key bs)
  in
  let* () =
    match kind with
...
let register_schema_from_fields (db : t) ~(kind : Props_capnp.schema_kind)
    ~(type_name : string) ~(data_word_count : int) ~(pointer_count : int)
  ...
  in
  let schema =
    make_registered_schema type_name kind data_word_count pointer_count fields
  in
  Hashtbl.replace schema_cache type_name schema;
  Ok schema
...
let load_all_schemas (db : t) : unit =
  ignore
    (Types.with_transaction_ro db (fun txn ->
