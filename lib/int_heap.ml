...
let rec sift_up_min h i =
  if i > 0 then begin
    let p = (i - 1) asr 1 in
    if h.dists.(i) < h.dists.(p) then begin
      swap h i p;
      sift_up_min h p
    end
  end
...
let rec sift_down_min h i =
  ...
  if l < h.size then begin
    let r = l + 1 in
    let smallest =
      if r < h.size && h.dists.(r) < h.dists.(l) then r else l
    in
    if h.dists.(smallest) < h.dists.(i) then begin
      swap h i smallest;
...
let rec sift_down_max h i =
  ...
  if l < h.size then begin
    ...
    let largest =
      if r < h.size && h.dists.(r) > h.dists.(l) then r else l
    in
    if h.dists.(largest) > h.dists.(i) then begin
      swap h i largest;
...
let peek_dist h =
  if h.size = 0 then
    match h.order with Min -> infinity | Max -> neg_infinity
  else h.dists.(0)
...
let pop h =
  if h.size = 0 then None
    ...
    if h.size > 0 then begin
      ...
      match h.order with
      | Min -> sift_down_min h 0
      | Max -> sift_down_max h 0
    end;
    Some (d, v)
  end
