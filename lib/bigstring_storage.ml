(** MessageStorage implementation for bigstrings. implements
    Capnp.MessageStorage.S for use with Capnp.Message.Make. *)
...
let zero_out buf ~pos ~len =
  let sub = Bigarray.Array1.sub buf pos len in
  Bigarray.Array1.fill sub '\x00'
