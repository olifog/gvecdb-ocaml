...
module CapnpWireFormat = struct
  ...
  let from_bytes (bs : bigstring) : (Capnp.Message.ro Bigstring_message.Message.t, error) result =
    ...
    try
      ...
      Ok (Bigstring_message.Message.readonly 
...
let register_node_schema (db : t) ?txn (type_name : string) (schema_id : int64) : (unit, error) result =
  wrap_lmdb_exn (fun () ->
...
let register_edge_schema (db : t) ?txn (type_name : string) (schema_id : int64) : (unit, error) result =
  wrap_lmdb_exn (fun () ->
    let key = "schema:" ^ type_name in
    let buf = Bigstring.create 9 in
...
let get_schema_metadata (db : t) ?txn (type_name : string) : (schema_kind * int64, error) result =
  let key = "schema:" ^ type_name in
  try
  ...
  | Not_found | Lmdb.Not_found -> Error (Storage_error ("schema not found: " ^ type_name))

let set_node_props_capnp
    (db : t) ?txn (node_id : node_id) (type_name : string)
    (build_fn : 'builder -> unit)
    (init_root : unit -> 'builder)
    (to_message : 'builder -> 'a Capnp.BytesMessage.Message.t) : (unit, error) result =
...
let get_edge_meta (db : t) ?txn (edge_id : edge_id) : ((intern_id * node_id * node_id), error) result =
  let key = Keys.encode_id_bs edge_id in
  try
    let meta = Lmdb.Map.get db.edge_meta ?txn key in
    Ok (Keys.decode_edge_meta meta)
