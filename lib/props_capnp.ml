...
(* Run a function with a read transaction. If a transaction is provided, use
   it. Otherwise create a new RO transaction that spans the entire callback.
...
let with_ro_txn (db : t) ?txn f =
  ...
  | None -> (
      ...
      | Some result -> result
      | None -> Error (Storage_error "read transaction aborted unexpectedly"))

module CapnpWireFormat = struct
  let to_bytes (message : 'a Capnp.BytesMessage.Message.t) : bigstring =
    let total_size = ref 0 in
    Capnp.Codecs.serialize_iter message ~compression:`None ~f:(fun fragment ->
        total_size := !total_size + String.length fragment);
  ...
  let parse_header (bs : bigstring) : (int array * int, error) result =
    try
      ...
      for i = 0 to segment_count - 1 do
        let word_count =
          Int32.to_int (Bigstring.get_int32_le bs (4 + (i * 4)))
        in
        segment_sizes.(i) <- word_count * 8
      done;
      let header_words = (segment_count + 2) / 2 in
      let data_offset = header_words * 8 in
      Ok (segment_sizes, data_offset)
    with _ -> Error (Corrupted_data "invalid capnp wire format header")

  (* Create BigstringMessage with segments pointing into the source bigstring. *)
  let from_bytes (bs : bigstring) :
      (Capnp.Message.ro Bigstring_message.Message.t, error) result =
    let* segment_sizes, data_offset = parse_header bs in
    try
      let segments =
        Array.make (Array.length segment_sizes) (Bigstring.create 0)
      in
    ...
    with _ -> Error (Corrupted_data "invalid capnp message segments")
end
...
let set_node_props_capnp (db : t) ?txn (node_id : node_id) (type_name : string)
    (build_fn : 'builder -> unit) (init_root : unit -> 'builder)
    (to_message : 'builder -> 'a Capnp.BytesMessage.Message.t) :
    (unit, error) result =
...
let get_node_props_capnp (db : t) ?txn (node_id : node_id)
    (of_message : Capnp.Message.ro Bigstring_message.Message.t -> 'reader)
    (read_fn : 'reader -> 'result) : ('result, error) result =
  with_ro_txn db ?txn (fun txn ->
      let key = Keys.encode_id_bs node_id in
      match
        try Some (Lmdb.Map.get db.nodes ~txn key)
        with Not_found | Lmdb.Not_found -> None
      with
      | None -> Error (Node_not_found node_id)
      | Some bytes ->
          let* message = CapnpWireFormat.from_bytes bytes in
          Ok (read_fn (of_message message)))

let set_edge_props_capnp (db : t) ?txn (edge_id : edge_id) (type_name : string)
    (build_fn : 'builder -> unit) (init_root : unit -> 'builder)
    (to_message : 'builder -> 'a Capnp.BytesMessage.Message.t) :
    (unit, error) result =
  let key = Keys.encode_id_bs edge_id in
  let edge_meta_result =
    try
  ...
  in
  let* _, src, dst = edge_meta_result in
  let* new_intern_id = Store.intern db ?txn type_name in
  wrap_lmdb_exn (fun () ->
      Lmdb.Map.set db.edge_meta ?txn key
        (Keys.encode_edge_meta ~type_id:new_intern_id ~src ~dst);
      let builder = init_root () in
      build_fn builder;
      let message = to_message builder in
      let bytes = CapnpWireFormat.to_bytes message in
...
let get_edge_props_capnp (db : t) ?txn (edge_id : edge_id)
    (of_message : Capnp.Message.ro Bigstring_message.Message.t -> 'reader)
    (read_fn : 'reader -> 'result) : ('result, error) result =
  with_ro_txn db ?txn (fun txn ->
      let key = Keys.encode_id_bs edge_id in
      match
        try Some (Lmdb.Map.get db.edges ~txn key)
        with Not_found | Lmdb.Not_found -> None
      with
      | None -> Error (Edge_not_found edge_id)
      | Some bytes ->
          let* message = CapnpWireFormat.from_bytes bytes in
          Ok (read_fn (of_message message)))
