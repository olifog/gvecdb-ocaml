...
let deserialize_field_value (s : string) : Gvecdb.Dynamic_reader.field_value =
  ...
  if len = 0 then V_void
    ...
    | 7 when len > 2 ->
        V_uint16 (Char.code s.[1] lor (Char.code s.[2] lsl 8))
    ...
    method register_schema_from_capnp_impl params release_param_caps =
      ...
      Fun.protect
        ...
        (fun () ->
          ...
          close_out oc;
          match Gvecdb.register_schema_from_capnp db ~kind ~type_name
              ~capnp_path:tmp ~struct_name () with
        ...
        Array.to_list
          (Array.map
             (fun fd ->
               ...
               let ft =
                 field_type_of_int (Api.Reader.FieldDescriptor.type_get fd) in
               let offset =
                 Stdint.Uint32.to_int
                   (Api.Reader.FieldDescriptor.offset_get fd) in
               let is_pointer =
                 Api.Reader.FieldDescriptor.is_pointer_get fd in
               { name; field_type = ft; offset; is_pointer;
      ...
      let filters =
        Array.to_list
          (Array.map
             (fun fp ->
               let field_name =
                 Api.Reader.FilterPredicate.field_name_get fp in
               let op =
               ...
               let value =
                 deserialize_field_value
                   (Api.Reader.FilterPredicate.value_get fp) in
               Gvecdb.Filter.{ field_name; op; value })
             filter_arr)
      in
      let edge_type_opt =
        if String.length edge_type = 0 then None else Some edge_type in
      let response, results =
        Capnp_rpc.Service.Response.create Results.init_pointer in
      (match
         Gvecdb.get_outbound_edges db node_id
           ?edge_type:edge_type_opt ~filters ()
       with
       | Ok edges -> set_edge_list results edges Results.edges_init
       | Error e -> Results.error_set results (Gvecdb.Error.to_string e));
      Capnp_rpc.Service.return response
    ...
    method create_vector_impl params release_param_caps =
      ...
      (match
         Gvecdb.with_transaction db (fun txn ->
           Gvecdb.create_vector db ~txn ~normalize ~metric Node node_id vector_tag
             bs)
       with
