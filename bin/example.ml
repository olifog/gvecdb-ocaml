...
let () =
  ...
  let alice_name = ok_exn (Gvecdb.get_node_props_capnp db alice
  ...
  let (alice_name, alice_age, alice_email, alice_bio) = 
    ok_exn (Gvecdb.get_node_props_capnp db alice
      SchemaReader.Reader.Person.of_message
      (fun reader ->
  ...
  ok_exn (Gvecdb.set_edge_props_capnp db edge1 "knows"
    (fun builder ->
  ...
  let (since, strength, context) = 
    ok_exn (Gvecdb.get_edge_props_capnp db edge1
  ...
  let result = Gvecdb.with_transaction db (fun txn ->
    ...
    ok_exn (Gvecdb.set_node_props_capnp db ~txn dave "person"
      (fun builder ->
