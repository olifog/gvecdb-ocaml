...
let test_id_roundtrip () =
  let test_values =
    [ 0L; 1L; 255L; 256L; 65535L; 65536L; Int64.max_int; Int64.min_int; -1L ]
  in
  List.iter
    (fun id ->
...
let test_adjacency_roundtrip () =
  ...
  let encoded =
    Keys.encode_adjacency_bs ~node_id ~intern_id ~opposite_id ~edge_id
  in
...
let test_adjacency_encoding_size () =
  let encoded =
    Keys.encode_adjacency_bs ~node_id:1L ~intern_id:2L ~opposite_id:3L
      ~edge_id:4L
  in
...
let test_adjacency_prefix_node_and_type () =
  let prefix =
    Keys.encode_adjacency_prefix_bs ~node_id:123L ~intern_id:456L ()
  in
...
let test_adjacency_key_ordering () =
  ...
  let k1 =
    Keys.encode_adjacency_bs ~node_id:1L ~intern_id:1L ~opposite_id:1L
      ~edge_id:1L
  in
  let k2 =
    Keys.encode_adjacency_bs ~node_id:1L ~intern_id:1L ~opposite_id:1L
  ...
  in
  let k3 =
    Keys.encode_adjacency_bs ~node_id:1L ~intern_id:1L ~opposite_id:2L
      ~edge_id:1L
  in
  let k4 =
    Keys.encode_adjacency_bs ~node_id:1L ~intern_id:2L ~opposite_id:1L
      ~edge_id:1L
  in
  let k5 =
    Keys.encode_adjacency_bs ~node_id:2L ~intern_id:1L ~opposite_id:1L
      ~edge_id:1L
  in
...
let test_has_prefix_true () =
  let key =
    Keys.encode_adjacency_bs ~node_id:123L ~intern_id:456L ~opposite_id:789L
      ~edge_id:101L
  in
  let prefix =
    Keys.encode_adjacency_prefix_bs ~node_id:123L ~intern_id:456L ()
...
let test_has_prefix_false () =
  ...
  check bool "key doesn't have wrong prefix" false
    (Keys.bigstring_has_prefix ~prefix:wrong_prefix key)

let test_has_prefix_equal_length () =
  let key = Keys.encode_id_bs 123L in
  let prefix = Keys.encode_id_bs 123L in
  check bool "equal length prefix match" true
    (Keys.bigstring_has_prefix ~prefix key)

let test_has_prefix_longer_prefix () =
  let key = Keys.encode_id_bs 123L in
  let prefix =
    Keys.encode_adjacency_bs ~node_id:123L ~intern_id:1L ~opposite_id:1L
      ~edge_id:1L
  in
  check bool "longer prefix doesn't match" false
    (Keys.bigstring_has_prefix ~prefix key)

let test_has_prefix_empty () =
  let key = Keys.encode_id_bs 123L in
  let empty = Bigstring.create 0 in
  check bool "empty prefix matches everything" true
    (Keys.bigstring_has_prefix ~prefix:empty key)

(** {1 Edge cases} *)

let test_zero_values () =
  let encoded =
    Keys.encode_adjacency_bs ~node_id:0L ~intern_id:0L ~opposite_id:0L
      ~edge_id:0L
  in
  let n, i, o, e = Keys.decode_adjacency_bs encoded in
...
let test_max_values () =
  ...
  let encoded =
    Keys.encode_adjacency_bs ~node_id:max ~intern_id:max ~opposite_id:max
      ~edge_id:max
  in
  let n, i, o, e = Keys.decode_adjacency_bs encoded in
