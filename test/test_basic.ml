...
let test_create_node () =
  ...
  check bool "node exists after creation" true (ok_exn (Gvecdb.node_exists db node))
...
let test_delete_node () =
  ...
  check bool "not exists after delete" false (ok_exn (Gvecdb.node_exists db node));
...
let test_delete_node_cascades_edges () =
  ...
  check int "alice outbound before" 1 (List.length (ok_exn (Gvecdb.get_outbound_edges db alice)));
  check int "alice inbound before" 1 (List.length (ok_exn (Gvecdb.get_inbound_edges db alice)));
  ok_exn (Gvecdb.delete_node db alice);
  check bool "alice not exists after" false (ok_exn (Gvecdb.node_exists db alice));
  ...
  check int "bob inbound after" 0 (List.length (ok_exn (Gvecdb.get_inbound_edges db bob)));
  check int "charlie outbound after" 0 (List.length (ok_exn (Gvecdb.get_outbound_edges db charlie)))
...
let test_delete_edge () =
  ...
  check bool "not exists after delete" false (ok_exn (Gvecdb.edge_exists db edge))
...
let test_delete_edge_cleans_adjacency () =
  ...
  let bob = ok_exn (Gvecdb.create_node db "person") in
  let edge = ok_exn (Gvecdb.create_edge db "knows" alice bob) in
  check int "outbound before" 1 (List.length (ok_exn (Gvecdb.get_outbound_edges db alice)));
  check int "inbound before" 1 (List.length (ok_exn (Gvecdb.get_inbound_edges db bob)));
  ok_exn (Gvecdb.delete_edge db edge);
  check int "outbound after" 0 (List.length (ok_exn (Gvecdb.get_outbound_edges db alice)));
  check int "inbound after" 0 (List.length (ok_exn (Gvecdb.get_inbound_edges db bob)))
...
let test_edge_props_roundtrip () =
  ...
  let (since, context, strength) = ok_exn (Gvecdb.get_edge_props_capnp db edge
...
let test_update_node_props () =
  ...
  ok_exn (Gvecdb.set_node_props_capnp db alice "person"
    (fun b ->
...
let test_persistence_across_reopen () =
  ...
  Fun.protect ~finally:(fun () -> try Sys.remove path with _ -> ()) (fun () ->
    ...
    Fun.protect ~finally:(fun () -> Gvecdb.close db) (fun () ->
      check bool "alice exists" true (ok_exn (Gvecdb.node_exists db alice_id));
