(** Zero-copy semantics and memory safety tests.
    ...
    1. Integer/struct reads don't copy the entire message
    2. Text fields do copy (expected capnp-ocaml behavior)
    3. Data is safely accessed within transactions
    4. Reading multiple fields shares the same backing memory *)
...
let test_integer_field_no_message_copy () =
  ...
  let (age, alloc) = measure_alloc (fun () ->
    get_person_age db node
  ) in
  
  check (testable (fun fmt a -> Format.fprintf fmt "%s" (Stdint.Uint32.to_string a)) 
