...
let output_results ~n ~dim ~metric ~k ~n_queries ~seed ~hnsw_params
    ~build_time ~peak_before ~peak_after ~index_size ~results ~output_dir =
  ...
  let filename = Filename.concat output_dir
    (Printf.sprintf "ann_%s_%d_%dd_k%d_m%d_%s.json"
      (metric_to_string metric) n dim k hnsw_params.Gvecdb.Hnsw.m ts) in
...
let () =
  ...
  let vectors, actual_dim, dataset_metric =
    if dataset_path <> "" then begin
      ...
      let meta = load_dataset_metadata
        (Filename.concat dataset_path "metadata.txt") in
      let metric = match Hashtbl.find_opt meta "metric" with
      ...
      (generate_dataset ~seed ~n ~dim, dim, None)
  in
  ...
  let ground_truth_ext =
    if dataset_path <> "" && actual_n = dataset_full_size then begin
  ...
  let k_values =
    ...
    if k_str <> "" then
      List.map int_of_string (String.split_on_char ',' k_str)
    else
      [get_int_arg "k" 10]
  in

  let metrics = match dataset_metric with
  ...
  List.iter (fun metric ->
    ...
    let (db, path, build_time, peak_before, peak_after) =
      build_index ~vectors ~dim:actual_dim ~metric ~hnsw_params
        ~prefix:(Printf.sprintf "ann_%d_%d" actual_n actual_dim) in

    Fun.protect ~finally:(fun () -> Gvecdb.close db; cleanup_db_files path) (fun () ->
      let index_size = get_db_size_bytes path in
      let peak_delta_kb = peak_after - peak_before in
      let bytes_per_vec = if actual_n > 0 then index_size / actual_n else 0 in
      Printf.printf "Build: %.2fs (%.0f vec/s), size: %d bytes (%d B/vec), peak RSS delta: %d KB\n%!"
        build_time (float actual_n /. build_time) index_size bytes_per_vec peak_delta_kb;

      Gc.compact ();

      List.iter (fun k ->
        Printf.printf "\n--- Sweep k=%d metric=%s ---\n%!" k (metric_to_string metric);
        let ground_truth =
