(** Concurrent Reader Isolation Test
    ...
    Validates MVCC reader isolation under true parallelism using OCaml
    Domains (OS-level threads). Measures:
...
let measure_read_latencies db queries ~n_samples =
  ...
  Array.init n_samples (fun i ->
    ...
    let (_, lat_us) = time_us (fun () ->
      ok_exn (Gvecdb.knn_hnsw db ~metric:Gvecdb.Euclidean
        ~k ~ef ~vector_tag:"v" q)) in
    lat_us)
...
let () =
  ...
  let write_vectors = Array.init n_write_vectors (fun _ -> random_vector_from rng dim) in
  ...
  Fun.protect ~finally:(fun () -> Gvecdb.close db; cleanup_db_files path) (fun () ->
    ...
    for qi = 0 to min 50 (n_queries - 1) do
      ignore (ok_exn (Gvecdb.knn_hnsw db ~metric:Gvecdb.Euclidean
        ~k ~ef ~vector_tag:"v" queries.(qi)))
    done;

    (* Phase 1: read-only latency (single domain, no contention) *)
    Printf.printf "Phase 1: Measuring read-only latency (%d queries)...\n%!" n_read_samples;
    let readonly_lats = measure_read_latencies db queries ~n_samples:n_read_samples in
    ...
    Gc.compact ();
    Printf.printf "Phase 2: Measuring read latency with concurrent writer domain...\n%!";
    ...
    let writer_domain = Domain.spawn (fun () ->
      ...
      while not (Atomic.get writer_done) && !wi < n_write_vectors do
        with_txn db (fun txn ->
          let node = ok_exn (Gvecdb.create_node db ~txn "doc") in
          ignore (ok_exn (Gvecdb.create_vector db ~txn ~metric:Gvecdb.Euclidean
            Node node "v" (floats_to_bigstring write_vectors.(!wi)))));
        incr wi;
    ...
    let concurrent_lats = measure_read_latencies db queries ~n_samples:n_read_samples in
    ...
    Printf.printf "  Concurrent: p50=%.0fus p99=%.0fus QPS=%.0f (writer inserted %d)\n%!"
      concurrent_stats.p50_us concurrent_stats.p99_us concurrent_stats.qps vectors_written;
    ...
    Printf.printf "  %s\n%!"
      (if p50_ratio < 1.5 then "PASS: reads not degraded (MVCC isolation confirmed)"
    ...
    let filename = Filename.concat output_dir
      (Printf.sprintf "concurrent_reader_%s.json" ts) in
