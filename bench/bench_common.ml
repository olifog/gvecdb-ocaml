...
let with_suppressed_gc f =
  Gc.compact ();
  let old = Gc.get () in
  Gc.set { old with minor_heap_size = 4_194_304; space_overhead = 1_000_000 };
...
let compute_recall ~ground_truth ~approximate =
  let gt_set =
    List.fold_left (fun s id -> Int64Set.add id s) Int64Set.empty ground_truth
  in
  ...
  if n_gt = 0 then 1.0
    ...
    let matches =
      List.fold_left
        (fun acc id -> if Int64Set.mem id gt_set then acc + 1 else acc)
        0 approximate
    in
...
(** {1 Dataset loading}
    ...
    Binary format (.fbin): int32_le n, int32_le dim, n*dim float32_le. Ground
    truth (.ibin): int32_le n_queries, int32_le k, n_queries*k int32_le. *)
    ...
    (fun () ->
      ...
      let gt =
        Array.init n (fun _ ->
            ...
            Array.init k (fun j ->
                Bytes.get_int32_le row_buf (j * 4) |> Int32.to_int))
      in
      (gt, k))
...
let get_db_size_bytes path =
  ...
  (try
     if Sys.file_exists hnsw_dir && Sys.is_directory hnsw_dir then
       Array.iter
         (fun f -> add (Filename.concat hnsw_dir f))
         (Sys.readdir hnsw_dir)
   with _ -> ());
  !total
...
let timestamp () =
  ...
  Printf.sprintf "%04d%02d%02d_%02d%02d%02d" (t.tm_year + 1900) (t.tm_mon + 1)
