(** Property Storage Benchmark
    ...
    Demonstrates zero-copy CapnProto reads: reading an integer field should have
    the same latency regardless of how large the backing blob is, because
    segments point directly into LMDB's mmap. *)
    ...
    Array.init n (fun i ->
        ...
        let (), lat =
          time_us (fun () ->
              ...
              let bs =
                capnp_to_bigstring SchemaBuilder.Builder.Person.to_message
                  builder
              in
    ...
    Array.init (n * 2) (fun i ->
        ...
        let _, lat =
          time_us (fun () ->
              ...
              let stream =
                Capnp.Codecs.FramedStream.of_string ~compression:`None s
              in
...
let () =
  ...
  let write_results =
    Array.map (fun bio_size -> (bio_size, bench_write ~n ~bio_size)) bio_sizes
  in

  let int_read_results =
    Array.map
      (fun bio_size -> (bio_size, bench_int_read ~n ~bio_size))
      bio_sizes
  in

  let full_read_results =
    Array.map
      (fun bio_size -> (bio_size, bench_full_read ~n ~bio_size))
      bio_sizes
  in
    ...
    `List
      (Array.to_list
         (Array.map
            (fun (bio_size, stats) ->
              `Assoc
                [ ("bio_size", `Int bio_size); ("stats", stats_to_json stats) ])
            results))
  in
  ...
  in
  let filename =
    Filename.concat output_dir (Printf.sprintf "props_%d_%s.json" n ts)
  in
