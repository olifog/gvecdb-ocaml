(** Crash Injection Test
    ...
    3. SIGKILL the child mid-batch (after some HNSW commits but before the LMDB
       transaction commits)
    ...
    The key crash scenario: each create_vector does HNSW commit (durable via
    msync) then buffers LMDB updates in the transaction. The LMDB transaction
    commits only when with_txn returns. Killing mid-batch leaves HNSW ahead of
    LMDB — the exact scenario reconciliation handles. *)
...
let () =
  ...
  let pre_vectors =
    Array.init n_pre_crash (fun _ -> random_vector_from rng dim)
  in
  ...
  while !i < n_pre_crash do
    ...
    with_txn db (fun txn ->
        for j = !i to batch_end - 1 do
          ...
          ignore
            (ok_exn
               (Gvecdb.create_vector db ~txn ~metric:Gvecdb.Euclidean Node node
                  "v"
                  (floats_to_bigstring pre_vectors.(j))))
        done);
    i := batch_end
  done;
  ...
  let pre_results =
    ok_exn
      (Gvecdb.knn_hnsw db ~metric:Gvecdb.Euclidean ~k ~ef ~vector_tag:"v" query)
  in
             ...
             ignore
               (ok_exn
                  (Gvecdb.create_vector db ~txn ~metric:Gvecdb.Euclidean Node
                     node "v" (floats_to_bigstring vec)));
             if j = signal_after then (
  ...
  (match status with
  | Unix.WSIGNALED s -> Printf.printf "  Child killed by signal %d\n%!" s
  | Unix.WEXITED c ->
  ...
  Printf.printf
    "  Reopened in %.3fs (includes reconciliation if epoch mismatch)\n%!"
    reopen_time;
  ...
  for i = 0 to check_count - 1 do
    ...
    match Gvecdb.node_exists db node_id with Ok true -> incr found | _ -> ()
  done;
  ...
  let post_search_all =
    ok_exn
      (Gvecdb.knn_hnsw db ~metric:Gvecdb.Euclidean ~k:n_pre_crash
         ~ef:n_pre_crash ~vector_tag:"v" query)
  in
  ...
  let all_passed = List.length post_results = k && !found = check_count in
  Printf.printf "\n=== Result: %s ===\n%!"
        ...
        ( "crash",
          `Assoc
            [
              ...
              ( "scenario",
                `String "mid-batch: HNSW committed, LMDB txn uncommitted" );
            ] );
        ( "recovery",
          `Assoc
            [
  ...
  in
  let filename =
    Filename.concat output_dir (Printf.sprintf "crash_injection_%s.json" ts)
  in
