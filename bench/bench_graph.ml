(** Graph Operations Benchmark
    ...
    Measures node/edge creation throughput, adjacency query latency, and cascade
    deletion cost. *)
...
let bench_node_creation ~n =
  ...
  let latencies =
    Array.init n (fun i ->
        let _, lat =
          time_us (fun () -> ok_exn (Gvecdb.create_node db "person"))
        in
...
let bench_edge_creation ~n_nodes ~n_edges ~seed =
  ...
  let nodes =
    Array.init n_nodes (fun _ -> ok_exn (Gvecdb.create_node db "person"))
  in
  ...
  let latencies =
    Array.init n_edges (fun i ->
        ...
        let etype =
          edge_types.(Random.State.int rng (Array.length edge_types))
        in
        let _, lat =
          time_us (fun () -> ok_exn (Gvecdb.create_edge db etype src dst))
        in
...
let bench_adjacency_queries ~n_nodes ~n_edges ~n_queries ~seed =
  ...
  let outbound_lats =
    Array.init n_queries (fun i ->
        ...
        let _, lat =
          time_us (fun () ->
              ignore (ok_exn (Gvecdb.get_outbound_edges db node ())))
        in
  ...
  let inbound_lats =
    Array.init n_queries (fun i ->
        ...
        let _, lat =
          time_us (fun () ->
              ignore (ok_exn (Gvecdb.get_inbound_edges db node ())))
        in
  ...
  let typed_lats =
    Array.init n_queries (fun i ->
        let node = nodes.(Random.State.int query_rng n_nodes) in
        let etype =
          edge_types.(Random.State.int query_rng (Array.length edge_types))
        in
        let _, lat =
          time_us (fun () ->
              ignore
                (ok_exn (Gvecdb.get_outbound_edges db node ~edge_type:etype ())))
        in
  ...
  Array.map
    (fun (edges_per, vecs_per) ->
      ...
      let latencies =
        Array.map
          (fun node ->
            snd (time_us (fun () -> ok_exn (Gvecdb.delete_node db node))))
          targets
      in
      let stats = compute_stats latencies in
      Printf.printf "  edges=%d vecs=%d: mean=%.0fus p95=%.0fus\n%!" edges_per
        vecs_per stats.mean_us stats.p95_us;
...
let () =
  ...
  let outbound, inbound, typed =
    bench_adjacency_queries ~n_nodes ~n_edges ~n_queries ~seed
  in
  ...
  let json : Yojson.Basic.t =
    `Assoc
      [
        ...
        ( "node_creation",
          `Assoc [ ("n", `Int n_nodes); ("stats", stats_to_json node_stats) ] );
        ( "edge_creation",
          `Assoc
            [
              ("n_nodes", `Int n_nodes);
  ...
  in
  let filename =
    Filename.concat output_dir (Printf.sprintf "graph_%d_%s.json" n_nodes ts)
  in
