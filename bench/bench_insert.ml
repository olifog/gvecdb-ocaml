(** Insertion Throughput Benchmark
    ...
    3. Rebuild throughput (rebuild_hnsw_index commits every 1000 vectors to HNSW)
...
(** Insert vectors normally, then measure rebuild_hnsw_index throughput
    (commits every 1000 vectors to HNSW) separately *)
